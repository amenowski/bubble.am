...
async function getNickNameFromHash(userHash) {
  try {
    // Zapytanie do bazy danych Supabase
    ...
    // Jeśli znaleziono dane, zwróć nick gracza
    if (data && data.length > 0) {
      return data[0].nick;
    } else {
      // W przeciwnym razie zwróć pustą wartość
      return "";
    }
  } catch (error) {
    console.error("Error fetching nickname from Supabase:", error.message);
    return ""; // Zwróć pustą wartość w przypadku błędu
  }
}

function PacketHandler(gameServer, socket, userHash) {
...
PacketHandler.prototype.handleMessage = function (message) {
  ...
  switch (packetId) {
    case 0:
      getNickNameFromHash(this.userHash)
        .then((nick) => {
          this.setNickname(nick); // Ustawienie nicku gracza na podstawie hasha
        })
        .catch((error) => {
      ...
      if (this.gameServer.config.serverAdminPass != "") {
        ...
        if (message.substr(0, passkey.length) == passkey) {
          ...
          console.log(
            "\u001B[36m" +
              wname +
              ": \u001B[0missued a remote console command: " +
              cmd
          );
          var split = cmd.split(" "),
          ...
          console.log(
            "\u001B[36m" +
              wname +
              ": \u001B[0missued a remote console command but used a wrong pass key!"
          );
          break;
        }
      }
