...
Commands.list = {
  help: function (gameServer, split) {
    console.log("========================== HELP ============================");
    console.log(
      "\u001B[36maddbot     \u001B[0m: add one or more bot to the server"
    );
    ...
    console.log(
      "\u001B[36mtp         \u001B[0m: teleport player to specified location"
    );
    console.log("\u001B[36munban      \u001B[0m: un ban a player with IP");
    console.log(
      "\u001B[36mvirus      \u001B[0m: spawn virus at a specified Location"
    );
    console.log("============================================================");
  },
  ...
  banlist: function (gameServer, split) {
    ...
    console.log(
      "\u001B[36mServer: \u001B[0mCurrent banned IPs (" +
        gameServer.banned.length +
        ")"
    );
  ...
  board: function (gameServer, split) {
    ...
    console.log(
      "\u001B[36mServer: \u001B[0mSuccessfully changed leaderboard values"
    );
  },
  ...
  food: function (gameServer, split) {
    ...
    console.log(
      "\u001B[36mServer: \u001B[0mSpawned 1 food cell at (" +
        pos.x +
        " , " +
        pos.y +
        ")"
  ...
  gamemode: function (gameServer, split) {
    try {
      ...
      console.log(
        "\u001B[36mServer: \u001B[0mChanged game mode to " +
          gameServer.gameMode.name
      );
    } catch (e) {
  ...
  merge: function (gameServer, split) {
    ...
    for (var i in gameServer.clients) {
      if (gameServer.clients[i].playerTracker.pID == id) {
        ...
        console.log(
          "\u001B[36mServer: \u001B[0mForced " + client.name + " to merge cells"
        );
        break;
      }
    }
  ...
  split: function (gameServer, split) {
    ...
    for (var i in gameServer.clients) {
      if (gameServer.clients[i].playerTracker.pID == id) {
        ...
        console.log(
          "\u001B[36mServer: \u001B[0mForced " + client.name + " to split cells"
        );
        break;
      }
    }
  ...
  playerlist: function (gameServer, split) {
    ...
    console.log(
      " ID         | IP              | " +
        fillChar("NICK", " ", gameServer.config.playerMaxNickLength + 2) +
        " | CELLS | SCORE  | POSITION    "
    ); // Fill space
    console.log(
      fillChar(
        "",
        ...
        " ID         | IP              |  | CELLS | SCORE  | POSITION    "
          .length +
          (gameServer.config.playerMaxNickLength + 2)
      )
    );
    for (var i = 0; i < gameServer.clients.length; i++) {
      var client = gameServer.clients[i].playerTracker;
      ...
      if (client.disconnect >= 0) {
        ...
        try {
          ...
          if (gameServer.getMode().specByLeaderboard) {
            ...
            nick =
              gameServer.clients[client.spectatedPlayer].playerTracker.name;
          }
        } catch (e) {
          // Specating nobody
          nick = "";
        ...
        nick = fillChar(
          client.name == "" ? "No Player Selected" : client.name,
          " ",
          gameServer.config.playerMaxNickLength + 2
        );
        ...
        position =
          fillChar(client.centerPos.x.toFixed(0), " ", 5, true) +
          ", " +
          fillChar(client.centerPos.y.toFixed(0), " ", 5, true);
        console.log(
          " " +
            id +
            " | " +
  ...
  reload: function (gameServer) {
    ...
    console.log(
      "\u001B[36mServer: \u001B[0mReloaded the config file successfully"
    );
  },
  status: function (gameServer, split) {
    ...
    console.log(
      "\u001B[36mServer: \u001B[0mConnected players: " +
        players +
        "/" +
        gameServer.config.serverMaxConnections
    );
    console.log(
      "\u001B[36mServer: \u001B[0mPlayers: " + humans + " Bots: " + bots
    );
    console.log(
      "\u001B[36mServer: \u001B[0mServer has been running for " +
        seconds2time(process.uptime())
    );

    var used = (process.memoryUsage().heapUsed / 1024).toFixed(0);
    var total = (process.memoryUsage().heapTotal / 1024).toFixed(0);
    console.log(
      "\u001B[36mServer: \u001B[0mCurrent memory usage: " +
        used +
        "/" +
        total +
        " Kb"
  ...
  virus: function (gameServer, split) {
    ...
    console.log(
      "\u001B[36mServer: \u001B[0mSpawned 1 virus at (" +
        pos.x +
        " , " +
        pos.y +
        ")"
