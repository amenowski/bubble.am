...
function Tournament() {
  ...
  this.prepTime = 3; // Amount of ticks after the server fills up to wait until starting the game
...
Tournament.prototype.onPlayerSpawn = async function (gameServer, player) {
  // Tylko spawnujemy graczy, jeśli gra jeszcze się nie rozpoczęła
  if (this.gamePhase === 0 && this.contenders.length < this.maxContenders) {
    // Pobieramy kolor gracza z bazy danych Supabase
    const playerColor = await getPlayerColorFromDatabase(player);

    if (playerColor && playerColor !== "") {
      // Jeśli kolor gracza jest dostępny w bazie danych, przypisujemy go
      player.color = playerColor;
    } else {
      // Jeśli kolor gracza nie jest dostępny w bazie danych lub jest pusty, przypisujemy losowy kolor
      player.color = gameServer.getRandomColor();
    }

    // Dodajemy gracza do listy contenders i spawnujemy go
    this.contenders.push(player);
    gameServer.spawnPlayer(player);

    // Jeśli liczba graczy osiągnie maksymalną liczbę, rozpoczynamy przygotowania do gry
