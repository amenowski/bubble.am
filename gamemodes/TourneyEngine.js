...
TourneyEngine.prototype.endGame = function () {
  ...
  if (players.length > 1 && this.scoreMode !== 2)
    for (var i = 0; i < players.length; i++) players[i].frozen = true;
};
...
TourneyEngine.prototype.onEndGame = function (gameServer) {
  ...
  if (this.reJoinInterval) clearTimeout(reJoinTimerHolder);

  while (players.length > 0) {
    // onCellRemove is making sure to keep track on alivePlayers
...
TourneyEngine.prototype.reSetupArena = function (gameServer) {
  ...
  while (this.nodesMother.length) gameServer.removeNode(this.nodesMother[0]);

  // ORIGINALLY TAKEN FROM gameServer.spawnCells()
  // spawn food at random size
  var spawnCount =
    gameServer.config.foodMinAmount - gameServer.nodesFood.length;
  for (var i = 0; i < spawnCount; i++) {
    var cell = new Entity.Food(
      gameServer,
      null,
      gameServer.getRandomPosition(),
      gameServer.config.foodMinSize
    );
  ...
  while (gameServer.nodesVirus.length < gameServer.config.virusMinAmount) {
    var virus = new Entity.Virus(
      ...
      null,
      gameServer.getRandomPosition(),
      gameServer.config.virusMinSize
    );
...
TourneyEngine.prototype.setupExperimental = function (gameServer) {
  ...
  TourneyEngine.prototype.spawnMotherCell = function (gameServer) {
    ...
    var mother = new Entity.MotherCell(
      gameServer,
      null,
      gameServer.getRandomPosition(),
      149
    );
    if (!gameServer.willCollide(mother)) gameServer.addNode(mother);
  };
...
TourneyEngine.prototype.onServerInit = function (gameServer) {
  ...
  if (this.scoreMode === 2)
    // 2 = Teams (remember, doesn't mean that there are Team Mode mechanics on)
  ...
  if (sleepMode < 2) return;

  setTimeout(function () {
    if (!sleepExecute) return;
...
TourneyEngine.prototype.onPlayerDC = function (gameServer) {
  ...
  for (var i = 0; i < gameServer.clients.length; i++)
    if (gameServer.clients[i].playerTracker.socket.isConnected) return;

  if (
    sleepMode == 2 &&
    gameServer.clients[0].playerTracker.socket._closeCode == 1000 &&
    ++this.countDC >= this.DCLimit
  ) {
  ...
  setTimeout(
    ...
    sleepMode == 2 || this.stage > 0
      ? (this.joinInterval + this.restartInterval) * 1000
      : 0 // To ensure server will not stuck in the middle of restarting or something
  );
};
...
TourneyEngine.prototype.onCellRemove = function (cell) {
  ...
  if (index !== -1) this.alivePlayers.splice(index, 1);
  ...
  if (this.scoreMode === 1) owner.deaths++;

  if (this.scoreMode === 2 && this.alivePlayers.length <= 3) {
    for (var i = 0; i < this.alivePlayers.length; i++)
      if (this.alivePlayers[i].team === owner.team) return;
...
TourneyEngine.prototype.updateKill = function (player) {
  ...
  if (index !== -1) this.alivePlayers[index].kills++;
};

TourneyEngine.prototype.onPlayerSpawn = function (gameServer, player) {
  if (gameServer.disableSpawn || player.isRemoved) return;
  ...
  player.team =
    this.minPlayers === 4
      ? this.alivePlayers.length >= 2
        ...
        : 0 // 2v2
      : this.minPlayers === 6
      ? this.alivePlayers.length >= 4
        ? 2
        : this.alivePlayers.length >= 2
        ? 1
        : 0 // 2v2v2
      : this.alivePlayers.length >= 6
      ? 2
      : this.alivePlayers.length >= 4
      ? 1
      : 0; // 3v3v3
  player.color =
    this.scoreMode === 2
      ? this.getTeamColor(player.team)
      : gameServer.getRandomColor();
  ...
  var index =
    this.minPlayers === 4
      ? this.alivePlayers.length >= 2
        ...
        : this.alivePlayers.length // 2v2
      : this.minPlayers === 6
      ? this.alivePlayers.length >= 4
        ? this.alivePlayers.length + 2
        : this.alivePlayers.length >= 2
        ? this.alivePlayers.length + 1
        : this.alivePlayers.length // 2v2v2
  ...
  if (this.alivePlayers.length >= this.minPlayers && this.stage < 2)
    // Waiting for min number of players to trigger actual game start
...
TourneyEngine.prototype.updateLB = function (gameServer, lb) {
  // Check if lb is undefined and initialize it as an empty array if it is
  ...
  switch (this.stage) {
    ...
    default: // Case 0: IDLE game stage or 1 player is waiting
      ...
      lb[1] =
        ...
        "/" +
        (this.minPlayers != gameServer.config.serverMaxConnections
          ? this.minPlayers + "-" + gameServer.config.serverMaxConnections
          : gameServer.config.serverMaxConnections);
      break;
  }
};
