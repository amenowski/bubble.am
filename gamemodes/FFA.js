...
FFA.prototype.onPlayerSpawn = async function (gameServer, player) {
  // Random color
  player.color = await gameServer.getPlayerColor(player);

  console.log(player.color);
  if (!player.color) player.color = gameServer.getRandomColor();
  console.log(player.color);
...
FFA.prototype.updateLB = function (gameServer) {
  ...
  for (var i = 0; i < gameServer.clients.length; i++) {
    ...
    if (lb.length == 0) {
      ...
      if (
        playerScore > lb[gameServer.config.gameLBlength - 1].getScore(false)
      ) {
        lb.pop();
        this.leaderboardAddSort(player, lb);
      }
