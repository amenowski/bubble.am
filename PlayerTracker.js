...
function stats2file(gameServer) {
  ...
  for (var i = 0; i < gameServer.clients.length; i++) {
    ...
    if (client.hscore > 0 && client.cscore > 0) {
      nick = fillChar(
        client.name == "" ? "An unnamed cell" : client.name,
        " ",
        gameServer.config.playerMaxNickLength
      );
      ...
      if (gameServer.sqlconfig.host != "" && ip != "BOT" && score > 100)
        gameServer.mysql.writeScore(
          nick,
          ip,
          score,
          gameServer.sqlconfig.table
        );
    }
  }
  wstream.end();
...
PlayerTracker.prototype.update = function () {
  ...
  this.socket.sendPacket(
    new Packet.UpdateNodes(
      this.nodeDestroyQueue,
      updateNodes,
      nonVisibleNodes,
      this.gameServer.config.serverVersion
    )
  );
...
PlayerTracker.prototype.getSpectateNodes = function () {
  ...
  if (this.gameServer.getMode().specByLeaderboard) {
    this.spectatedPlayer = Math.min(
      this.gameServer.leaderboard.length - 1,
      this.spectatedPlayer
    );
    specPlayer =
      this.spectatedPlayer == -1
        ? null
        : this.gameServer.leaderboard[this.spectatedPlayer];
  } else {
    this.spectatedPlayer = Math.min(
      this.gameServer.clients.length - 1,
      this.spectatedPlayer
    );
    specPlayer =
      this.spectatedPlayer == -1
        ? null
        : this.gameServer.clients[this.spectatedPlayer].playerTracker;
  }
  ...
  if (specPlayer) {
    ...
    this.socket.sendPacket(
      new Packet.UpdatePosition(
        specPlayer.centerPos.x,
        specPlayer.centerPos.y,
        specZoom
      )
    );
