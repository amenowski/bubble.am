(function (wHandle, wjQuery) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("gamemode").addEventListener("change", function () {
      ...
      if (this.value == "localhost:2345" || this.value == "localhost:1234") {
        console.log(this.value);
        switch (this.value) {
          case "localhost:2345":
            document.querySelector("#radio_mode .radio-battle").style.display =
              "block";
            break;
          default:
            document.querySelector("#radio_mode .radio-battle").style.display =
        ...
        document.getElementById("radio_mode").style.display = "block";
      } else {
        document.getElementById("radio_mode").style.display = "none";
      }

      if (this.value === "localhost:2345") {
        if (
          !document.getElementById("play-btn").classList.contains("btn-danger")
        ) {
          document.getElementById("play-btn").classList.add("btn-danger");
        }

        if (
          document.getElementById("play-btn").classList.contains("btn-primary")
        ) {
          document.getElementById("play-btn").classList.remove("btn-primary");
        }

        // if(logged) {
        // 	document.querySelectorAll('.showFriends').forEach(function(element) {
        // 		element.style.display = 'none';
        // 	});
        // 	document.getElementById('playBtn').style.width = '86%';
        // }
      } else {
        document.getElementById("play-btn").style.width = "85%";
        document.querySelectorAll(".showFriends").forEach(function (element) {
        ...
        if (
          ...
          document.getElementById("play-btn").classList.remove("btn-danger");
        }
  ...
  function httpGet(theUrl) {
    ...
    xmlHttp.send(null);
    console.log("Response from " + url + ":", xmlHttp.responseText); // Dodaj ten wiersz
  ...
  function drawChatBoard() {
    ...
    var state2 = 0;
    var isAdminAmen = chatNick.trim() === "amen";
    console.log(isAdminAmen, chatNick);
    if (chatNick == "[system]") {
      if (chatNick == "[system]") {
        chatNick = "";
      ...
      chatNick =
        '<strong style="color:' +
        chatBoard[len - 1].color +
        '">' +
        chatNick +
        "</strong> ";
    }

    // Check if the sender is an admin with the nickname "amen"

    // // Set background color based on admin status
    var backgroundColor = isAdminAmen ? "#FFAAAA" : "transparent";

    wjQuery("#chatLog").append(
      `<div style="background-color: ${backgroundColor}" class="mess_` +
        chatClassID +
        " mess_type_" +
        state2 +
        '">' +
  ...
  function sendUserHash() {
    if (wsIsOpen() && userHash) {
      var msg = prepareData(1 + 2 * userHash.length);
      msg.setUint8(0, 0);
      for (var i = 0; i < userHash.length; ++i)
        msg.setUint16(1 + 2 * i, userHash.charCodeAt(i), true);
      wsSend(msg);
    }
  }
      ...
      if (drawTeam || showName) {
        ...
        if (!drawTeam) {
          for (
            ...
            if (!noRanking) c = b + 1 + ". " + c; // Zmiana tutaj, dodanie b+1 jako numeracji
            var start =
      ...
      if (drawTeam || showName) {
        ...
        if (!drawTeam) {
          for (
            ...
            ctx.fillText(
              name,
              100 - ctx.measureText(name).width * 0.5,
              70 + 24 * b
            );
          }
        } else {
