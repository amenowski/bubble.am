...
Cell.prototype.addMass = function (n) {
  if (
    this.mass + n > this.owner.gameServer.config.playerMaxMass &&
    this.owner.cells.length < this.owner.gameServer.config.playerMaxCells
  ) {
    ...
    this.mass = Math.min(
      this.mass + n,
      this.owner.gameServer.config.playerMaxMass
    );
  }
};
